...
public class CountrySorting extends TestBase {
  ...
  public void geoZonesSorting() {
    ...
    List<WebElement> rows = app.driver.findElements(By.cssSelector("form[name=countries_form] .dataTable .row"));
    ...
    for (String zone : zones){
      app.driver.navigate().to(zone);
      List<WebElement> geozones = app.driver.findElements(By.cssSelector(".dataTable tr td:nth-child(3) input[type='hidden']"));
  ...
  public void geoZonePageSorting(){
    AdminLoginTest login = new AdminLoginTest();
    login.adminLoginTest();
    app.driver.navigate().to("http://localhost:8080/litecart/admin/?app=geo_zones&doc=geo_zones");
    List<WebElement> geozones = app.driver.findElements(By.cssSelector(".dataTable .row td:nth-child(3) a"));
    List<String> pageLinks = new ArrayList<>();
    for (WebElement geozone : geozones){
      pageLinks.add(geozone.getAttribute("href"));
    }
    for (String pagelink : pageLinks){
      app.driver.navigate().to(pagelink);
      List<String> geozonenames = new ArrayList<>();
      List<String> geozonenamessorted = new ArrayList<>();
      List<WebElement> geoZoneWeb = app.driver.findElements(By.cssSelector("#table-zones select[name*='zone_code'] option[selected='selected']"));
